...
for i=1:length(num)
    ...
    color_rgb=num(i,:);
    color_mat=repmat(color_rgb,[10 10 3]);
     %color_rgb=reshape(color_rgb,1,1,3);
    %figure(1);cla;imagesc(color_rgb);
    figure(1);cla;imagesc(uint8(color_mat));
    newColorNames{i}=input('new name? :','s')
    num(i,:)=num(i,:)/norm(num(i,:));
end

save([svm_path 'newColorNames.mat'],'newColorNames');
